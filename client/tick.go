...
func connect(addr string) (*handler, error) {
	...
	go func() {
		err := h.heartbeatloop()
		if err != nil {
			logrus.Error("heartbeatloop - ", err)
		}
	}()
	return h, nil
}
...
func (h *handler) SendText(msg string) error {
	...
	return wsutil.WriteClientMessage(h.conn, ws.OpText, []byte(msg))
}
